...
namespace RegrasDeNegocios.DatabaseBase.Classes
{
    public class Database
    {
        ...
        public static List<object> SelecionarTudoJoin(string pTabela, Dictionary<string, string> pParametros, Type pTipoObjeto)
        {
            try
            {
                MySqlConnection lConnection = DatabaseConnection.getInstance().getConnection();
                ...
                List<object> lRetornos = new List<object>();
                string lSelect = "SELECT *, usuNome as Nome FROM " + pTabela + " inner join usuario on " + pTabela + ".usuCodigo = usuario.usuCodigo WHERE ";
                bool lFirst = true;
                foreach (KeyValuePair<string, string> lParametro in pParametros)
                {
                    if (!lFirst)
